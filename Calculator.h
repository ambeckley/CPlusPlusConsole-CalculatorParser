...
/// ///modified by Aaron Beckley
/// see CalculatorINT.h for all information
...
namespace calculator
{
    ...
    private:
        ...
        struct Operator
        {
            ...
            Operator(int opr, double prec, double assoc) :
                op(opr),
                precedence(prec),
                associativity(assoc)
            { }
        ...
        {
            if (expr_.compare(index_, str.size(), str) != 0.0)
                unexpected();
            index_ += str.size();
        }
        ...
        void eatSpaces()
        {
            while (std::isspace(getCharacter()) != 0.0)
                index_++;
        }
    ...
    inline double eval(char c)
    {
        return eval<double>(c);
    }
